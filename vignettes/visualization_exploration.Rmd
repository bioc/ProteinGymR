...
By default, if no `data_source` argument is provided, the DMS substitutions from 
...
`supervised_available_model()`. If no `data_source` argument is specified, DMS
scores are plotted. Alternatively, a user can specify their own DMS list to 
project. 
...
regions we do not have assay information for rather than using the 
"ball and stick" representation, we can set the argument 
`full_structure == TRUE`. Let's also plot model scores from the semi-supervised
model called "Kermut" by setting the `data_scores` argument.
...
Rather than using the default "AlphaMissense", get model for semi-supervised
Kermut model.
...
and "EVE_single" models as defaults.
...
Between the GEMME and EVE_ensemble, there seems to be good correlation between
