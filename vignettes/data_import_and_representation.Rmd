...
Datasets in `ProteinGymR` can be easily loaded in with built-in functions.
...
For this vignette, we are subset to the first 100 positions. 
...
Based on the AUC metric of evaluation, GEMME performed the best while 
CARP_600K performed the worst of the 5 selected models. If the `metric` argument
is not defined, the default used is a Spearman correlation. For more information 
about the models and metrics, see the function documentation 
`?benchmark_models()`.
