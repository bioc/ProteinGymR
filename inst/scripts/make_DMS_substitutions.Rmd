...
ProGym_tables <- purrr::map2(ProGym_tables, ProGym_names, ~ {
  .x %>%
    mutate(DMS_id = .y)
})
```
...
# Grab PG study names without Hobbs -- Used later
...
ProGym_tables <- purrr::map2(ProGym_tables, uniprot_vec, ~ {
    .x %>%
    mutate(UniProt_id = .y)
})
...
```{r, eval=FALSE}
saveRDS(ProGym_tables, "ProGym216_DMS_subs_v2.rds")
```
