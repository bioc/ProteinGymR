...
2. The same `list` as above for 17 additional models (total of 79).
...
As of Bioc 3.21, the latest data contains 79 models downloaded from 
ProteinGym [v1.2][Zenodo_v1.2] (accessed 2025-03-26).
...
```{r, eval = FALSE}
# Download from Zenodo - data stored locally
file_dir <- paste0("/home/rstudio/ProteinGym_data/ProteinGym_v1.2/",
    "DMS_benchmarks_performance/DMS_zero_shot/substitutions/")

ProGym_files <- list.files(path=file_dir, 
    pattern = "DMS_level.csv$", full.names=FALSE, recursive = TRUE)

# Grab assay names
ProGym_names <- sub("/.*", "", ProGym_files) 
...
# Replace all punctuation with underscores in column names
...
score_list <- lapply(ProGym_tables, change_colnames)
...
score_list <- lapply(score_list, function(df) {
  names(df) <- gsub("_$", "", names(df))
  return(df)
}) # remove hanging underscores at end

# Check that each dataframe has same column and row names across the list
all(sapply(score_list[-1], 
    function(df) identical(names(df), names(score_list[[1]]))))
all(sapply(score_list[-1], 
    function(df) identical(df[,1], score_list[[1]][,1])))

## DISCREPENCIES BETWEEN MODEL NAMES IN FILES
## CHANGE TO MODEL NAMES PROVIDED IN `available_models()`


# Check that the names match the raw scores model names
available_models() == names(score_list[[1]])[2:80]
chk <- cbind(available_models(), names(score_list[[1]])[2:80])

# Loop through the list and change column names from 2 to 80
# Replace model names

score_list <- lapply(score_list, function(df) {
  colnames(df)[2:80] <- available_models()
  df
})
...
# Write model score metrics summary RDS object

```{r, eval=FALSE}
saveRDS(score_list, 
    "/home/rstudio/ProteinGym_data/EH_data/v1.2/zeroshot_summary_scores_v1.2.rds")
```
